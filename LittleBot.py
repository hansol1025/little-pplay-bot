...
async def on_message(message):
	if message.content.startswith('리틀아' and '누구야'):
	...
	else if message.content.startswith('리틀아' and '양비카' and '생각해'):
		...
		await client.send_message(message.channel, cc)
	else if message.content.startswith('리틀아' and '현솔' and '생각해'):
		cc = random.choice(['현솔님은 정말 좋은 분이셔..', '안녕 리틀이라고 해요!', '안뇽~ 리틀이에요 요즘 학교 때문에 너무 힘드네요 ㅠ..', '안녕! 반가워.. 내가 누구냐고? 그러게 나도 내가 누군지 모르겠어 ..', '안녕, 리틀이라고 해 요즘 공부 너무 힘들다', '난 리틀이라고 해..', '누군긴 누구야 리틀이지'])
		await client.send_message(message.channel, cc)
